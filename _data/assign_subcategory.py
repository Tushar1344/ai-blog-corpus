...
SUB_RULES = {
    "pretraining-and-architecture": [
        ...
        ("research-techniques-and-methods", r"\b(emotion concept|behavioral differences|diff tool for ai|language model (behaviour|behavior)|model behaviour|consistency model|energy-based|generative (model|qa)|word embedding|text embedding|contrastive pre|masked (autoencoder|pretrain)|self-supervised|meta.?learning|few-shot|zero-shot|in-context learning|\bicl\b|retrieval.?pretrain|long range memory)\b"),
    ...
    "alignment-and-safety": [
        ...
        ("deceptive-alignment", r"\b(alignment faking|deceptive alignment|sleeper agent|sandbag|scheming|treacherous|mesa.?optim|model organism|latent misalign|encoded reasoning|hidden reasoning|trojan|backdoor|poison|model poisoning|insider threat|agentic misalignment)\b"),
        ("red-teaming-and-jailbreaking", r"\b(red[- ]?team|jailbreak|jail-break|adversarial prompt|adversarial suffix|many-shot jailbreak|universal jailbreak|prompt injection|injection attack|glitch token|prefix attack|auditing tool|petri\b)\b"),
        ("capability-evals-for-safety", r"\b(bioweapon|biosecur|\bcbrn\b|cyber (eval|capability|offens|defender)|persuasion eval|autonomy (eval|capability)|self.?exfiltration|situational awareness|dangerous capability|agi evals|uplift eval|sabotage eval)\b"),
        ("scalable-oversight", r"\b(scalable oversight|iterated amplification|debate\b|recursive reward|weak[- ]to[- ]strong|factored cognition|amplification|ai safety via debate|super.?human (oversight|assist)|influence function)\b"),
        ("responsible-scaling-and-policy", r"\b(\brsp\b|responsible scaling|preparedness framework|frontier safety framework|model spec|safety policy|governance|responsible ai|accountability|policy enforcement|responsible disclosure|transparency report|safeguard policy|\baup\b|core views|commitments on|economic impact|deployment polic|societal impact|labor market|economic index)\b"),
        ("constitutional-and-self-critique", r"\b(constitutional ai|constitutional approach|constitutional classifier|self[- ]critique|ai-written critique|claude\'s character|character training|\bpersona\b|self[- ]correction|model welfare)\b"),
        ("reward-hacking-and-sycophancy", r"\b(reward hack|reward gaming|specification gaming|sycophan|mode collapse|exploration collapse|prover[- ]verifier|faithful(ness)? of|chain.of.thought monitor|monitor(ability|ing)|eval awareness|honest|truthful|disempowerment|shortcut|lie\b|lies\b|lying)\b"),
        ("safety-research-general", r"\b(safety (research|training|techniques|eval)|alignment (research|lab|technique)|general (language|ai) assistant|harm reduction|toxicity|bias|fair(ness)?|responsible|ethics|ethical)\b"),
    ...
    "evals-and-benchmarks": [
        ...
        ("eval-systems-and-harnesses", r"\b(demystifying eval|evaluation framework|eval harness|evaluation platform|eval platform|continuous evaluation|offline eval|online eval|hybrid eval|benchmark suite|eval at scale|evalu(ating|ations?) (ai|llm|models?|claude|gpt)|\bevals? for\b|measurement framework)\b"),
        ("domain-specific-evals", r"\b(science eval|medical eval|legal eval|clinical eval|biological eval|chemistry eval|math problem|legal benchmark|healthcare benchmark|domain benchmark|enterprise eval|classification|ranking|truthful|truthfulqa|truthfulness|evalu(ating|ations?) (bias|fairness|safety|alignment|reasoning|capability|performance|ability|scientific|real.world))\b"),
        ("fallback-evals", r".*"),
    ],
    ...
    "agentic-systems": [
        ...
        ("agent-design-and-patterns", r"\b(building (effective|better) (ai )?agents?|trustworthy agents?|agent best practices|agent architecture|agent pattern|effective agent|real.world agent|production agent|reliable agent|agent design|designing agents?|mle.?bench)\b"),
        ("agents-general", r"\b(agent\b|agentic|autonomous|\bai assistant\b|autonomous systems)\b"),  # broad catch-all before fallback
        ("fallback-agents", r".*"),
    ],
    "harness-and-context-engineering": [
        ...
        ("codex-and-sora-harness", r"\b(codex harness|unrolling (the )?codex|codex agent|beyond rate limits|codex app server|sora for|build (chatgpt|atlas)|owl\b|c compiler|parallel claudes|postmortem|recent issues|scaling postgresql|in-house data agent)\b"),
        ("fallback-harness", r".*"),
    ],
}
...
SUB_TITLES = {
    ...
    "domain-specific-evals": "Domain-specific evals (science, medical, legal)",
    ...
    "agent-design-and-patterns": "Agent design & patterns",
...
SUB_ORDER = {
    "pretraining-and-architecture": [
        ...
        "research-techniques-and-methods", "societal-impact-and-deployment-studies",
    ...
    "alignment-and-safety": [
        "responsible-scaling-and-policy", "constitutional-and-self-critique",
    ...
    "evals-and-benchmarks": [
        ...
        "judge-models-and-methodology", "eval-systems-and-harnesses",
    ...
    "agentic-systems": [
        ...
        "embodied-and-simulation", "agent-traces-and-observability",
        "agent-design-and-patterns", "agents-general",
        "fallback-agents",
    ],
    "harness-and-context-engineering": [
        "context-engineering", "long-running-harnesses", "mcp-and-tool-protocols",
        "scaffolding-patterns", "agent-skills-and-prompt-libraries",
        "prompt-caching", "codex-and-sora-harness",
        "fallback-harness",
    ],
}
